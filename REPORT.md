...
# 开发报告 (第 18 轮) — README 校准

## 本轮新增

1. **README 校准** — 刷新"已实现"表到当前完整功能集(attachment/发送重试/重启补发/启动通知/待确认问题内容等); 修正了一处过时表述(第 2 轮曾写"dsh 不重放问题", 第 3 轮已实证 dsh 会重放, 现按实证修正)。
...
1. **`verify_all.py` 加 `--pytest`** — 一条命令跑完整"发布体检": 单测(85) → 环境自检 → 确认流程演示, 全过才退出 0。以后改完配置/代码跑一次就能确认交付物健康。

## 第 19 轮实测结果

- ✅ `verify_all --pytest --auto 4` 全绿: 85 单测 + 环境 OK + 确认流程 OK
...
1. ✅ 85/85 单测通过
2. ✅ `verify_all.py --auto 4` 发布体检通过(环境 OK + 确认流程 OK)
3. ✅ 真实 dsh 端到端: 建会话 → prompt → agent 回复 `OK` → `turn/end` 通知转发
4. ✅ 测试会话已归档, git 工作区干净

交付物当前处于就绪状态。等你的拍板项/凭据即可联调真实飞书/QQ。
...
# 开发报告 (第 24 轮) — Ubuntu 回归(跨平台再确认)

## 本轮新增

1. **Ubuntu (WSL2) 再回归** — 距第 10 轮首次 Ubuntu 验证后又改了十几轮(新增发送重试、attachment、verify_all --pytest、--test-notify 等), 本轮在 Ubuntu 24.04 / Python 3.12 上重跑全部测试: **85/85 通过**, 确认所有改动没有破坏 Linux 兼容性。Windows 侧同样全过。

## 第 24 轮实测结果

- ✅ Ubuntu: 85/85 单测通过
- ✅ Windows: 85/85 单测通过 + `--check` OK
...
1. **`scripts/wps_relay.py` 公网中继** — 解决 WOA 真实联调的最大部署难点：WPS 平台要能访问回调地址，而桥接核心必须贴着内网的 dsh。
   ...
   - 回复仍由桥接核心直接出站 HTTPS 发回 WPS（NAT 后面也能发）。
   - 用法、systemd 部署写进 `INTEGRATION.md`。
3. 新增 3 个中继测试：Challenge 应答、验签解密后转发到桥接核心、坏签名丢弃。

## 第 32 轮实测结果

- ✅ 101/101 单测通过（新增 3 个中继测试）
- ✅ 中继"接收→验签→解密→转发"全链路测试锁定

## 仍待你提供（WOA 真实联调）

WPS 开放平台建应用后给我：App ID、Secret Key、Encrypt Key（若有）、API 地址（日本平台实际 apiUrl）。回调部署方案三选一（公网 IP / 内网穿透 / wps_relay 公网中继），告诉我选哪个即可。
...
# 开发报告 (第 34 轮) — WOA 消息去重

## 本轮新增(测试 101 → **102 个**)

1. **WOA 接收消息去重** — WPS 若因网络抖动/响应慢重试 webhook，同一消息可能被重复投递（你的消息会重复发给 dsh）。现按消息 ID 去重（内存 LRU，上限 2000、TTL 1 小时），重复事件直接跳过。
2. 新增去重测试：同一消息投递两次，只处理一次。

## 第 34 轮实测结果

- ✅ 102/102 单测通过（WOA 相关 14 个）
- ✅ WOA 发送/接收/加解密/去重/中继全路径测试锁定
...
# 开发报告 (第 48 轮) — 飞书 + QQ 端到端演示

## 本轮新增

1. **`scripts/demo_im.py` 飞书 + QQ 端到端演示**（无凭据）— 用假服务器 + **真实通道的真实接收路径**（飞书长连接 Challenge 握手 + 事件、QQ OneBot WS 消息）+ 真实 hub + 假 dsh，验证完整链路：平台消息 → 通道接收 → hub → dsh → 回复发回平台。实测：两条消息各进 dsh 一次，飞书/QQ 回复都正确发回。
2. 修复演示中发现的小问题：飞书假 WS 重连会重发事件（演示侧改为只发一次）；回复打印时机/结构。

## 第 48 轮实测结果

- ✅ `demo_im.py` 全链路跑通（exit 0）
- ✅ 飞书、QQ 收发链路（含真实接收路径）验证
- ✅ 104/104 单测通过

## 仍待你提供（飞书/QQ 真实联调）

填 `CREDENTIALS.md`：飞书（webhook URL 或 app_id/app_secret）+ QQ（OneBot WS 地址/机器人号）+ 拍板项。
