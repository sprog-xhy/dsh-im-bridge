"""dsh-qq 插件测试：验证入口强制 QQ 官方单通道、默认配置指向插件自带模板。"""

import sys
from pathlib import Path

_PLUGIN_ROOT = Path(__file__).resolve().parents[1]   # plugins/dsh-qq
...
import dsh_qq.__main__ as qq_main
from dsh_qq.__main__ import CHANNEL, DEFAULT_CONFIG, PLUGIN_DIR


def test_channel_is_qq_official():
    assert CHANNEL == "qq_official"
...
def test_main_injects_only_channel_when_missing(monkeypatch):
    ...
    monkeypatch.setattr(qq_main, "bridge_main", fake_bridge_main)
    qq_main.main(["--config", "my.yaml"])  # no --only
...
def test_main_keeps_user_only(monkeypatch):
    ...
    monkeypatch.setattr(qq_main, "bridge_main", fake_bridge_main)
    qq_main.main(["--config", "my.yaml", "--only", "feishu"])
    assert captured["argv"][captured["argv"].index("--only") + 1] == "feishu"  # 尊重用户


def test_plugin_config_only_enables_qq_official():
    ...
    # the plugin template enables the QQ official channel
    assert cfg.channels["qq_official"]["enabled"] is True
    # runtime --only filtering leaves exactly the target channel
    cfg = _apply_only(cfg, "qq_official")
    assert set(cfg.channels) == {"qq_official"}
